...
const ProductAddDialog = ({ open, onClose, onSave }) => {
  ...
  const validationSchema = Yup.object({
    ...
    file: Yup.mixed().required('File is required').test('fileSize', 'File is too large', (value) => value && value.size <= 2000000), // 2MB max
  });
  ...
  const formik = useFormik({
    ...
    // In the Formik onSubmit function:
const formData = new FormData();
formData.append('file', values.file);  // Ensure this matches the backend 'file' field name
...
onClose();
  ...
  const handleFileChange = (e) => {
    ...
    formik.setFieldValue('file', selectedFile); // Manually update Formik field value
  };
...
<FormControl fullWidth margin="normal">
  <Input
    ...
    fullWidth
    sx={{ marginTop: 2 }}  // Optional margin for spacing
  />
